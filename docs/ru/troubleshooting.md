...
## Команды консоли, не заходя в игру

У сервера есть консоль, и `dstfarm` уже с ней разговаривает — так уходит команда штатного
завершения. Тем же путём можно отправить любую другую:

```
dstfarm console "AllPlayers[1].components.builder:GiveAllRecipes()"
```

Эта открывает подключённому игроку все рецепты, так что научная машина не нужна вовсе.
Другие полезные:

```
dstfarm console "AllPlayers[1].components.health:SetPercent(1)"
dstfarm console "AllPlayers[1].components.hunger:SetPercent(1)"
dstfarm console "c_announce(\"привет\")"
```

Команды идут через файл-очередь, потому что stdin сервера держит процесс супервизора.
Сервер должен быть запущен, результат смотрите в логе сервера.
...
## Освободить те самые 4 ГБ

Серверная сборка занимает около 4.2 ГБ и нужна не всегда:
...
С ключом `--all` уедет и steamcmd. Без `--yes` команда только покажет, что удалит и сколько
освободит.

Мир, токен кластера и `config.json` не трогаются, поэтому после `dstfarm install` всё
продолжится ровно с того же места.
